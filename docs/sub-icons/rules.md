...
- Declare `"iconType": "sub"` in every editable source and author directly on
  the sub design canvas. Recompose the subject for that canvas; never make a
  sub icon by scaling a finished normal icon.
- Prefer one dominant silhouette and no more than two identity-bearing internal
  features. Favor recognition at the sub ship size when choosing details; do
  not impose a count of geometry elements.
- Select a sub keyshape from the dominant whole-icon silhouette and declare
  `keyfitCheck.targetToken` explicitly. The sub circle and square share
  full-canvas painted bounds, so a bounding box alone cannot distinguish them.
...
- Use the sub profile's 3u minimum centerline distance for distinct parts.
  Shared R5's visual-opening and connection requirements still apply where
  relevant.
- Inspect recognition and negative space at the exact sub ship size in the
  generated profile.
...
At the current Regular 4u design stroke, centerlines at the 3u floor can have
...
collide. Deliver schema-version-2 editable JSON, the exact sub design/ship pair, and the
