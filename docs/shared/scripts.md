...
| [`core/rework_pack.py`](../../core/rework_pack.py) | Local manifest pack: `{inspect,prepare,build} PACK`; `--skip-qa` is diagnostic only. | Prepare writes prototype detection evidence/sheet. Build emits mapped version-2 sources, runs QA, checks reference and hash-locked visual review, and copies passing ship SVGs to manifest destinations. Writes fresh timestamped QA, `rework-results.json`, gallery and contact sheet. Never POSTs. | `0` inspect/prepare success or all-symbol build pass; `1` error, failing build or diagnostic-only build; `2` usage error. A failed run may contain passing local deliveries; inspect every result. | Full | Full | No |
...
| --- | --- | --- | --- | --- | --- | --- |
| [`core/generate_profile_assets.py`](../../core/generate_profile_assets.py) | `core/icon_profiles.json`; optional `--frontend`, `--docs`, `--type-docs-root`, and read-only `--check`. | Generates the browser registry, `docs/shared/icon-profiles.md`, and `profile.md` under each type folder. `--docs` changes the combined reference; `--type-docs-root` changes the root for all three type pages. Without `--check`, creates parents and rewrites stale mirrors. | `0` all five mirrors match or were generated; `1` at least one is missing or stale under `--check`; `2` usage error. Check mode writes nothing. | Full | Full | Full |
...
| [`core/render_icon.py`](../../core/render_icon.py) | Compatibility editable JSON and optional output filename; accepts compatibility-only `--distance-check`. | Requested profile-design SVG plus a ship companion suffixed with its profile size. | `0` rendered; nonzero on invalid input/geometry/write error; `2` usage error. Does not replace canonical validation. | Full | Full | Outer only |
...
| [`core/test_rework_pack.py`](../../core/test_rework_pack.py) | Manifest pack scope, safe local paths, editable-source build and delivery behavior. | Full | No | No |
