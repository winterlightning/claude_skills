(()=>{
  ...
  function choose(){row=rows.find(r=>r.id===$('pairSelect').value);if(!row)return;
    for(const [id,group]of [['pairMain','mains'],['pairSub','subs']])$(id).replaceChildren(...row[group].map(m=>option(m.icon,m.icon+(m.export_size===32?' · 32px reuse export':''))));
