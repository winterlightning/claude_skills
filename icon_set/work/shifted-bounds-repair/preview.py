...
for theme,bg,fg in [('light','#f7f6f2','#202226'),('dark','#17191d','#f1f2f5')]:
 im=Image.new('RGB',(1250,370),bg);d=ImageDraw.Draw(im);d.text((16,12),'ALL VALIDATION CHECKS PASS — parent / revised / native 48px',font=font,fill=fg)
 for i,r in enumerate(rows):
  ...
  d.text((x+8,280),'All checks PASS',font=font,fill=fg);d.text((x+8,304),'Including internal-spacing review',font=font,fill=fg)
...
(W/'index.html').write_text('<!doctype html><meta charset="utf-8"><title>Shifted bounds repairs</title><style>body{font:15px system-ui;background:#f7f6f2;color:#24272d;margin:28px}main{display:grid;grid-template-columns:repeat(auto-fit,minmax(300px,1fr));gap:18px}article{padding:18px;border:1px solid #ddd;border-radius:12px;background:white}h2{font-size:17px}pre{white-space:pre-wrap;font-size:12px}</style><h1>Five fully validated repairs</h1><p>All five revised variants pass model validation, gallery spacing and holes checks, and internal-spacing review. Parents are preserved. Full reports are in full-validation.json.</p><p><a href="preview-light.png">Light comparison</a> · <a href="preview-dark.png">Dark comparison</a></p><main>'+''.join(cards)+'</main>')
