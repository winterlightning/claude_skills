...
assert len(rows)==65 and all(r['qa']['negative_space']['status']=='pass' for r in rows)
passed=sum(r['qa']['status']=='pass' for r in rows)
parts=[]
for index,r in enumerate(rows,1):
 name=r['original'];q=r['qa'];fully=q['status']=='pass';findings=q['errors']+q['warnings']
 before=Path('icon_set/dist/failed/solo48',name+'.svg').read_text();after=(W/(r['candidate']+'.svg')).read_text()
 ...
 parts.append(f'''<article id="icon-{index}" data-name="{html.escape(name)}" data-status="{'pass' if fully else 'remaining'}">
 ...
 <div class="comparison">{figure(before,'Original')}{figure(after,'Proposed fix')}</div>
...
page='''<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>65 opening repairs — review</title>
<style>
...
<header><div class="eyebrow">Pictographic · SOLO48 · review batch</div><h1>Give the openings room.</h1><p>65 proposed repairs for the Undersized hole group. Compare the original and revised drawings, enlarged and at their actual 48-pixel size.</p><p><strong>Original sources preserved. These candidates have not been applied to the failure gallery.</strong> Opening checks pass for every candidate; remaining bounds and spacing findings are shown on each card. The live gallery was regenerated during this work and now lists 88 hole-failure icons; this batch covers the original 65 selected for repair.</p>
<div class="stats"><div><b>65 / 65</b><span>Opening checks pass</span></div><div><b>__PASS__</b><span>All blocking checks pass</span></div><div><b>__REMAIN__</b><span>Still have other findings</span></div></div></header>
<div class="toolbar"><div class="toolbar-inner"><input id="search" type="search" placeholder="Find an icon…" aria-label="Find an icon"><select id="filter" aria-label="Filter validation status"><option value="all">All 65 icons</option><option value="pass">All blocking checks pass</option><option value="remaining">Other findings remain</option></select><button id="theme" type="button">Dark theme</button><span id="count">65 icons</span></div></div>
...
function apply(){let n=0;for(const card of cards){const show=card.dataset.name.includes(search.value.toLowerCase().trim())&&(filter.value==='all'||card.dataset.status===filter.value);card.hidden=!show;if(show)n++}document.querySelector('#count').textContent=n+' icons'}search.addEventListener('input',apply);filter.addEventListener('change',apply);
...
(W/'review.html').write_text(page.replace('__PASS__',str(passed)).replace('__REMAIN__',str(65-passed)).replace('__CARDS__','\n'.join(parts)))
