...
for n,(shape,parts,ref,body) in D.items():
 ...
 if n==25:
  cl.body=[x for x in cl.body if not (isinstance(x,ast.Assign) and any(isinstance(y,ast.Name) and y.id=='human_construction' for y in x.targets))]
  # Standard detached user construction uses the shared four-unit ink gap.
