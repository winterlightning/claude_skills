"""waiting room couple.
Plan: Repair round1 enlarge clock and shorten hands, move seated pair lower; preserve exact detached human gap.
...
class Drawing(Solo48):
    ...
    def build(self):
        self.circle('clock',14,14,8)
        self.add_polyline('clock-hands',(14,12),(14,14),(16,14))
        for i,x in enumerate((25,39)):
            self.circle(f'head-{i}',x,22,3)
            self.add_line(f'torso-{i}',(x,33),(x,36))
            self.mark_human_figure(f'person-{i}',head=f'head-{i}',torso=f'torso-{i}',torso_junction='start')
        self.add_polyline('legs-left',(25,36),(16,36),(12,42));self.join('torso-0','legs-left')
