"""waiting room couple.
Plan: Repair round2 enlarge clock to fit hands and rebalance to smaller equal human heads; exact human gap retained.
...
class Drawing(Solo48):
    ...
    def build(self):
        self.circle('clock',17,17,11)
        self.add_polyline('clock-hands',(17,15),(17,17),(19,17))
        for i,x in enumerate((27,39)):
            self.circle(f'head-{i}',x,24,2)
            self.add_line(f'torso-{i}',(x,34),(x,36))
            self.mark_human_figure(f'person-{i}',head=f'head-{i}',torso=f'torso-{i}',torso_junction='start')
        self.add_polyline('legs-left',(27,36),(18,36),(14,42));self.join('torso-0','legs-left')
        self.add_polyline('legs-right',(39,36),(42,36),(42,42));self.join('torso-1','legs-right')
