"""waiting room couple.
Symbol plan: Paired seated stick figures, exact8 centerline head gaps; clock upper left. Shared full_body_ref.png anatomy.
...
class Drawing(Solo48):
    ...
    def build(self):
        ...
        for i,x in enumerate((27,41)):
            ...
            self.add_line(f'torso-{i}',(x,28),(x,34))
            self.mark_human_figure(f'person-{i}',head=f'head-{i}',torso=f'torso-{i}',torso_junction='start')
            self.add_polyline(f'legs-{i}',(x,34),(x-7,34),(x-7,42));self.join(f'torso-{i}',f'legs-{i}')
