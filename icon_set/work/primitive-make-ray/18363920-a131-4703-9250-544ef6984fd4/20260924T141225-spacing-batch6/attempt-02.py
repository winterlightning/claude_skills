"""waiting room couple.
Symbol plan: Wider envelope; clock opens toward seated figures. Human reference circular heads share radius3, upper torsos align at gap8.
...
class Drawing(Solo48):
    ...
    def build(self):
        self.add_arc('clock',(14,28),(14,8),radius_x=10,sweep=True)
        self.add_polyline('clock-hands',(14,17),(14,18),(17,18))
        for i,x in enumerate((27,41)):
            self.circle(f'head-{i}',x,17,3)
            self.add_line(f'torso-{i}',(x,28),(x,33))
            self.mark_human_figure(f'person-{i}',head=f'head-{i}',torso=f'torso-{i}',torso_junction='start')
            self.add_polyline(f'legs-{i}',(x,33),(x-6,33),(x-6,40));self.join(f'torso-{i}',f'legs-{i}')
