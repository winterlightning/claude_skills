...
class Drawing(Solo48):
    ...
    def build(self):
        ...
        self.path('front-body',(14,40),[('L',(14,36)),('A',(24,26),10,10,True),('A',(34,36),10,10,True),('L',(34,40))])
        for side in (-1,1):
            p=lambda x,y:(x,y) if side==-1 else (48-x,y)
            n='left' if side==-1 else 'right'
            self.circle(n+'-head',8 if side==-1 else 40,18,3)
            self.path(n+'-body',p(4,40),[('L',p(4,33)),('A',p(8,29),4,4,side==-1),('C',p(14,36),p(12,29),p(14,31))])
            self.relate('connect',n+'-body','front-body')
