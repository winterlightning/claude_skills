...
class Drawing(Solo48):
    ...
    def build(self):
        ...
        for i,(left,cx,right) in enumerate(((4,11,18),(18,24,30),(30,37,44))):
            ...
            curve(f'crown-left-{i}',(left,16),(left if i==0 else left+2,12),(cx-4,10),(cx,10))
            curve(f'crown-right-{i}',(cx,10),(cx+4,10),(right if i==2 else right-2,12),(right,16))
            curve(f'lower-left-{i}',(left,20),(left if i==0 else left+2,24),(cx-4,26),(cx,26))
            curve(f'lower-right-{i}',(cx,26),(cx+4,26),(right if i==2 else right-2,24),(right,20))
        for i,x in enumerate((4,18,30,44)):line(f'upper-side-{i}',(x,16),(x,20))
        for i,(left,cx,right) in enumerate(((11,18,24),(24,30,37))):
            curve(f'base-left-{i}',(left,30),(left if i==0 else left+2,35),(cx-4,38),(cx,38))
            curve(f'base-right-{i}',(cx,38),(cx+4,38),(right if i==1 else right-2,35),(right,30))
