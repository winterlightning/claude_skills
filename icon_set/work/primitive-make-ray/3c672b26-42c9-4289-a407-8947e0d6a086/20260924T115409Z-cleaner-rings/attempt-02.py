...
class Drawing(Solo48):
    ...
    def build(self):
        ...
        for i,(left,cx,right) in enumerate(((4,10,17),(17,24,31),(31,38,44))):
            curve(f'crown-left-{i}',(left,17),(left if i==0 else left+2,12),(cx-4,10),(cx,10))
            curve(f'crown-right-{i}',(cx,10),(cx+4,10),(right if i==2 else right-2,12),(right,17))
            curve(f'lower-left-{i}',(left,19),(left if i==0 else left+3,23),(cx-2,26),(cx,26))
            curve(f'lower-right-{i}',(cx,26),(cx+2,26),(right if i==2 else right-3,23),(right,19))
        for i,x in enumerate((4,17,31,44)):line(f'upper-side-{i}',(x,17),(x,19))
        for i,(left,cx,right) in enumerate(((10,17,24),(24,31,38))):
            curve(f'base-left-{i}',(left,29),(left if i==0 else left+3,34),(cx-4,38),(cx,38))
            curve(f'base-right-{i}',(cx,38),(cx+4,38),(right if i==1 else right-3,34),(right,29))
