"""Passport cover and globe: a mirrored meridian pair, equator and circular rim. Rounded enclosure uses the Lucide id-card corner construction."""
from icon_set.model.keyshapes import Keyshape
from icon_set.model.icons.solo._base import Solo48
SOURCE_ICON_ID = '1f180a84-a846-4671-b767-dbb6041f800a'
SOURCE_PATH = 'pictographic-primitives/travel/passport_1f180a84-a846-4671-b767-dbb6041f800a.svg'
AUTHOR = 'gpt-6'
PLAN = 'Passport cover with a globe emblem.'
OMISSIONS = 'Globe reduced to one straight meridian and equator; omitted the thin overlapping cover/page edge. Four large quadrants replace small crescents.'
class Drawing(Solo48):
    icon_id = 'passport'
    keyshape = Keyshape.SQUARE
    semantic_role = 'MAIN'
    semantic_kind = 'noun'
    category = 'objects'
    aliases = ()
    keywords = ('passport',)
    def build(self):
        # Passport cover with a globe reduced to four broad quadrants.
        self.add_polyline('cover',(6,6),(42,6),(42,42),(6,42),closed=True)
        self.circle('globe',24,24,9)
        self.add_polyline('meridian',(24,15),(24,24),(24,33))
        self.add_polyline('equator',(15,24),(24,24),(33,24))
        self.relate('connect','globe','meridian')
        self.relate('connect','globe','equator')
        self.relate('connect','meridian','equator')

    def circle(self,n,x,y,r):
        pts=[(x-r,y),(x,y-r),(x+r,y),(x,y+r),(x-r,y)]
        for i,(a,b) in enumerate(zip(pts,pts[1:])):
            self.add_arc(n+str(i),a,b,radius_x=r)
        self.add_contour(n,*(n+str(i) for i in range(4)),closed=True)

# Final repair review: Passport cover with a globe emblem.
# SQUARE maximizes the interior available for the globe.
# Changes: Globe reduced to one straight meridian and equator; omitted the thin overlapping cover/page edge. Four large quadrants replace small crescents.
# validate_icon: valid; build gate: pass with zero errors and zero warnings.
