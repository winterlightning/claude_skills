...
for theme,bg,ink in [('light','#f7f6f2','#17191d'),('dark','#17191d','#f7f6f2')]:
 for batch in range(6):
  im=Image.new('RGB',(1050,650),bg);d=ImageDraw.Draw(im)
  for k,r in enumerate(rs[batch*21:batch*21+21]):
   x=k%7*150;y=k//7*210;t=Image.open(io.BytesIO(render_png(create(r['id']),scale=1,ink=ink)));im.paste(t,(x+50,y+15),t);t=Image.open(io.BytesIO(render_png(create(r['id']),scale=2,ink=ink)));im.paste(t,(x+38,y+75),t)
   words=r['original'].split('-');line='';yy=y+160
   for word in words:
