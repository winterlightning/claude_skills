...
AUTHOR='gpt-6'
plans=json.loads((W/'plan.json').read_text());select=set(sys.argv[1:]);plans=[r for r in plans if not select or r['icon_id'] in select]
results=[]
for r in plans:
 try:
...
for page in range((len(plans)+7)//8):
 ...
 for j,r in enumerate(plans[page*8:page*8+8]):
  ...
  for k,p in enumerate([W/(r['icon_id']+'-before.svg'),W/(r['new_id']+'.svg')]):
