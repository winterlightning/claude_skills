"""Remove the short inner wing return so the swan reads as a broad curved body above a calm waterline. Independent feedback revision; parent preserved."""
...
class SwanOnWaterVariant2(Solo48):
    ...
    variant_label = 'Remove the short inner wing return so the swan reads as a broad curved body above a calm waterline.'
    ...
    def build(self) -> None:
        """Symbol plan: Remove the short inner wing return so the swan reads as a broad curved body above a calm waterline. Reference: Lucide bird: a reduced continuous neck and body."""
        ...
        self.add_contour('neck-wing', 'head-right', 'head-left', 'neck', 'neck-return')
