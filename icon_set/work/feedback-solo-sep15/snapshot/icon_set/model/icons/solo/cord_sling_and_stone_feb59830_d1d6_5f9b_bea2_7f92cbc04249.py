"""Cord Sling and Stone. Large cord loop, narrow hanging pouch stroke and separate stone; doubled cord omitted.
Keyshape VRECT_XL: chosen for the subject's overall proportions; authored directly on SOLO48.
"""
from ...keyshapes import Keyshape
from ._base import Solo48
SOURCE_ICON_ID = 'feb59830-d1d6-5f9b-bea2-7f92cbc04249'
SOURCE_PATH = 'pictographic-primitives/war/antique sling_feb59830-d1d6-5f9b-bea2-7f92cbc04249.svg'
AUTHOR = 'gpt-6'

class CordSlingAndStone(Solo48):
    icon_id = 'cord-sling-and-stone'
    keyshape = Keyshape.VRECT_XL
    semantic_role = 'MAIN'
    semantic_kind = 'noun'
    category = 'objects/war'
    aliases = ()
    keywords = ('sling', 'stone', 'cord', 'pouch', 'loop', 'weapon')

    def build(self):

        def L(n,a,b): self.add_line(n,a,b)
        def P(n,*p,closed=False): self.add_polyline(n,*p,closed=closed)
        def A(n,a,b,r,ry=None,s=True): self.add_arc(n,a,b,radius_x=r,radius_y=ry or r,sweep=s)
        def C(n,x,y,r):
            A(n+'a',(x-r,y),(x+r,y),r)
            A(n+'b',(x+r,y),(x-r,y),r)
            self.add_contour(n,n+'a',n+'b',closed=True)
        def J(a,b): self.relate('connect',a,b)
        def R(n,x,y,w,h,r=4):
            L(n+'t',(x+r,y),(x+w-r,y))
            A(n+'tr',(x+w-r,y),(x+w,y+r),r)
            L(n+'r',(x+w,y+r),(x+w,y+h-r))
            A(n+'br',(x+w,y+h-r),(x+w-r,y+h),r)
            L(n+'b',(x+w-r,y+h),(x+r,y+h))
            A(n+'bl',(x+r,y+h),(x,y+h-r),r)
            L(n+'l',(x,y+h-r),(x,y+r))
            A(n+'tl',(x,y+r),(x+r,y),r)
            self.add_contour(n,*[n+s for s in ('t','tr','r','br','b','bl','l','tl')],closed=True)

        C('loop',30,14,10)
        P('cord',(20,14),(8,35),(8,42));J('cord','loop')
        P('pouch-cord',(8,6),(8,17),(17,29));J('pouch-cord','cord')
        C('stone',33,38,6)
