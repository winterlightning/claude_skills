"""Shorten the four legs and replace the angular claws with opposing curves. Applied to the original icon identity."""
...
class Scorpion(Solo48):
    ...
    def build(self):
        ...
        for side in (-1, 1):
            ...
            line(f'upper-leg-{side}', (x(6), 27), (x(14), 25))
            join(f'upper-leg-{side}', 'body')
            line(f'lower-leg-{side}', (x(6), 27), (x(14), 36))
            join(f'lower-leg-{side}', 'body')
            path(f'arm-{side}', (24, 20), [('L', (x(12), 15))])
