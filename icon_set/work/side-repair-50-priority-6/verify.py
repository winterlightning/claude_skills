...
report=dict(reviewed=50,repaired=len(accepted),unresolved=50-len(accepted),parents_unchanged=50,source_files_unchanged=50,export_qa_pass=len(accepted),container_composition_pass=sum(r['status']=='valid' for r in load(W/'composition-checks.json').values()),wide_text_placement_pending=sum(r['status']=='not_applicable_wide_text' for r in load(W/'composition-checks.json').values()),pair_rows_updated=changed,independent_symbol_versions_unchanged=True,unrelated_canonical_records_unchanged=True,gallery_cards=p.cards,gallery_images=len(p.images),side_progress=summary)
