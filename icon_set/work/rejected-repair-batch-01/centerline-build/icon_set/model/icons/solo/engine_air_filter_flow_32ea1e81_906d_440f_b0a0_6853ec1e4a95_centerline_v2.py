"""Replace wavy filter edges with a clear repeated zigzag pleat; keep the two pleats identical and separate, with aligned inlet strokes.
...
class EngineAirFilterFlow(Solo48):
    icon_id = 'engine-air-filter-flow-centerline-v2'
    ...
    def build(self) -> None:
        for i, x in enumerate((18, 30)):
            self.add_line(f'pleat-{i}-a', (x, 8), (x - 2, 16))
            self.add_line(f'pleat-{i}-b', (x - 2, 16), (x, 24))
            self.add_line(f'pleat-{i}-c', (x, 24), (x + 2, 32))
            self.add_line(f'pleat-{i}-d', (x + 2, 32), (x, 40))
            self.add_contour(f'pleat-{i}', *[f'pleat-{i}-{s}' for s in 'abcd'])
        for y in (8, 40):
            self.add_polyline(f'cap-{y}', (16, y), (18, y), (30, y), (32, y))
            for i in range(2):
                self.relate('connect', f'cap-{y}', f'pleat-{i}')
        for i in range(3):
            y = 14 + i * 10
    ...
    variant_of = 'engine-air-filter-flow'
