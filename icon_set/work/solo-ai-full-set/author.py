...
HELPERS=HELPERS.replace('                if kind == "L":', '                if kind == "L" and tuple(end) == tuple(here):\n                    continue\n                if kind == "L":')
DESIGNS={}
...
def main():
 old={r['icon_id']:r for r in json.loads((ROOT/SOURCE_PATH).read_text())}
  ...
  for n in cls.body:
   if isinstance(n,ast.Assign):
    for t in n.targets:
     ...
     if isinstance(t,ast.Name) and t.id=='keywords':n.value=ast.Tuple([ast.Constant('solo-ai-full-set'),ast.Constant(name)],ast.Load())
  cls.body=[n for n in cls.body if not isinstance(n,ast.FunctionDef)]
  header=f'    def build(self):\n        # Plan: {plan}\n        # Reference: '+(f'Lucide {ref}: original and atomic-debug geometry.' if ref else 'Original subject; preserve the distinctive silhouette and proportions.')+'\n'
