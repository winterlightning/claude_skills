...
for row in findings:
 ...
 for issue in row['issues']:
  ...
  if not all(isinstance(p,Arc) and True for p in circle):continue
  geos=[arc_geometry(p) for p in circle];g=geos[0];cx,cy,r,ry=g.center_x,g.center_y,g.radius_x,g.radius_y
  if abs(cx-round(cx))+abs(cy-round(cy))+abs(r-round(r))>1e-6:continue
  if any(math.hypot(a.center_x-cx,a.center_y-cy)>1e-6 or abs(a.radius_x-r)>1e-6 or abs(a.radius_y-ry)>1e-6 for a in geos):continue
  x,y=issue['endpoint'];candidates=[]
  for xx in range(round(cx-r),round(cx+r)+1):
   for yy in range(round(cy-ry),round(cy+ry)+1):
    if (xx-round(cx))**2*round(ry)**2+(yy-round(cy))**2*round(r)**2==round(r)**2*round(ry)**2:candidates.append((xx,yy))
 ...
 for cid,points in contacts.items():
  for pid in groups[cid].members:
   ...
   for pt in points:
    ...
    a=math.atan2((pt[1]-g.center_y)/g.radius_y,(pt[0]-g.center_x)/g.radius_x)
 ...
 accepted.append(dict(id=id,source=rows[id]['source'],plan='Replace approximate ellipse attachments with exact integer boundary nodes, and expose each node in its receiving arc.',reference='Original subject; exact ellipse equation and shared junctions.',details=plans))
(H/'ellipse-contacts.json').write_text(json.dumps(accepted,indent=2));(H/'ellipse-contacts-deferred.json').write_text(json.dumps(deferred,indent=2,default=str));print('Exact circle contacts:',len(accepted),'Deferred:',len(deferred));print(', '.join(r['id'] for r in accepted))
