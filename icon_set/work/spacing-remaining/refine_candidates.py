...
def propose(n):
 ...
 for iteration in range(4):
  ...
  for g in sorted(active,key=len):
   ...
   ops += [(s,cx,cy,0,0) for s in (1.06,1.12,.94,.88) for cx,cy in ((owncx,owncy),(24,24))]
   for op in ops:
    trial=copy.deepcopy(i)
    moved={p.element_id:trans(p,*op) for p in i.primitives if p.element_id in g}
    pointmap={}
    for p in i.primitives:
     if p.element_id in g:
      q=moved[p.element_id];pointmap[p.start]=q.start;pointmap[p.end]=q.end
    trial.primitives=[]
    for p in i.primitives:
     if p.element_id in moved:trial.primitives.append(moved[p.element_id]);continue
     st=pointmap.get(p.start,p.start);en=pointmap.get(p.end,p.end)
     if isinstance(p,Bezier) and (st!=p.start or en!=p.end):
      seg=list(p.segments);a,b,c=seg[-1];seg[-1]=(a,b,en.as_tuple());trial.primitives.append(replace(p,start=st,end=en,segments=tuple(seg)))
     else:trial.primitives.append(replace(p,start=st,end=en))
    tc,tr=score(trial)
...
if __name__=='__main__':
 start=int(sys.argv[1]);end=int(sys.argv[2]);rs=json.load(open(W/'targets.json'));out=W/f'refine-{start:03}-{end:03}.json';results=[]
 previous={q['id']:q for p in W.glob('candidates-*.json') for q in json.load(open(p))}
 for row in rs[start:end]:
  if previous.get(row['id'],{}).get('score')==0:continue
