...
report=load(W/'verification.json');report.update(build_exit_code=0,built_models_verified=len(a),built_side_previews_verified=len(a),tests_passed=50,combined_svg_caches_regenerated=False)
(W/'verification.json').write_text(json.dumps(report,indent=2))
target=D/'gallery'/W.name;target.mkdir(parents=True,exist_ok=True)
for name in ('index.html','audit.json','verification.json','composition-checks.json','text-verification.json','README.md'):
