...
for r in accepted:
 ...
 assert qa[str(r['number'])]['model_sha256']==hashlib.sha256((ROOT/r['candidate_python']).read_bytes()).hexdigest()
...
p=Parser();p.feed((W/'index.html').read_text());assert p.cards==18 and len(p.images)==108
for src in p.images:assert (W/src).is_file(),src
summary=load(ROOT/'icon_set/work/side-repair-priority/summary.json');before=load(W/'summary-before.json')['statuses']; expected=dict(before)
for r in accepted:
 expected[r['status']]-=1; expected['pass']+=1
assert summary['statuses']==expected,(summary['statuses'],expected)
report=dict(reviewed=len(audit),repaired=len(accepted),unresolved=len(audit)-len(accepted),parents_unchanged=len(audit),source_files_unchanged=len(audit),export_qa_pass=len(accepted),container_composition_pass=sum(r['status']=='valid' for r in load(W/'composition-checks.json').values()),wide_text_placement_pending=sum(r['status']=='not_applicable_wide_text' for r in load(W/'composition-checks.json').values()),pair_rows_updated=changed,independent_symbol_versions_unchanged=True,unrelated_canonical_records_unchanged=True,gallery_cards=p.cards,gallery_images=len(p.images),side_progress=summary)
