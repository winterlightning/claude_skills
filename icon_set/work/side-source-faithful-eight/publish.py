...
with output_lock(dist):
 ...
 for r in accepted:
  ...
  record.update(canvas_width=width,canvas_height=height,sizing_mode=getattr(m,'sizing_mode',None),side_only=getattr(m,'sizing_mode',None) in ('side-32x48','side-one-axis32'))
