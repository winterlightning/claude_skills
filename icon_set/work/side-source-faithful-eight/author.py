...
from designs import D,GLYPHS,TEXT,TALL,DIMS
...
HELPERS=HELPERS.replace('    # One rounded rectangle', '    if k == 0:\n        s.add_polyline(n,(l,t),(r,t),(r,b),(l,b),(l,t));return\n    # One rounded rectangle')
...
for n,(shape,parts,ref,body) in D.items():
 ...
 t.body=[x for x in t.body if not (isinstance(x,ast.Assign) and any(isinstance(y,ast.Name) and y.id=='TYPEFACE_PROFILE_VARIANTS' for y in x.targets))]
 if n in TALL:
  for node in t.body:
   if isinstance(node,ast.ImportFrom) and node.module=='_base':node.module='_tall_base';node.names=[ast.alias('TallSideSub32')]
  cl.bases=[ast.Name('TallSideSub32',ast.Load())]
 if n in DIMS:
  for node in t.body:
   if isinstance(node,ast.ImportFrom) and node.module in ('_base','_tall_base'):node.module='_tall_base';node.names=[ast.alias('SideSub32Exception')]
  cl.bases=[ast.Name('SideSub32Exception',ast.Load())]
  cl.body+=ast.parse(f'canvas_width = {DIMS[n][0]}\ncanvas_height = {DIMS[n][1]}').body
 if n in TEXT:
  spec=TEXT[n]
  for node in t.body:
