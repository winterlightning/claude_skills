...
for offset in range(0,50,10):
 im=Image.new('RGB',(1200,1100),'white');d=ImageDraw.Draw(im)
 for j in range(10):
  k=str(offset+j+1);x=(j%2)*600;y=(j//2)*220
  ...
  for doc,xx,sz,dark in [(svg,x+10,150,False),(center,x+210,150,False),(svg,x+425,32,False),(svg,x+495,32,True)]:
   if dark:
    tree=ET.fromstring(doc)
    for el in tree.iter():
     if el.get('stroke') and el.get('stroke')!='none':el.set('stroke','white')
    doc=ET.tostring(tree).decode();d.rectangle((xx-10,y+70,xx+42,y+122),fill='#15252a')
   png=cairosvg.svg2png(bytestring=doc.encode(),output_width=sz,output_height=sz);p=Image.open(io.BytesIO(png)).convert('RGBA');im.paste(p,(xx,y+80 if sz==32 else y+50),p)
