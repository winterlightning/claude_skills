...
for r in json.loads((W/'queue.json').read_text()):
 p,i,s=prepare_variant(r['id'],'solo','Circle envelope and full spacing repair');m=re.search(r"SOURCE_ICON_ID = ['\"]([^'\"]+)",s)
 if m:p=p.with_name(p.stem+'_'+m[1].replace('-','_')+'.py')
 s=re.sub(r"AUTHOR = .*","AUTHOR = 'gpt-6'",s)
 with p.open('x') as f:f.write(s)
 mapping.append({'original':r['id'],'id':i,'file':str(p.relative_to(ROOT))});registry._FACTORIES=None
