...
for(const [number,text] of specs){
 ...
 try{
  const result=layout(text,glyphs,{canvasHeight:number===39?16:32,stroke:4,padding:0,trimInk:true,align:'center'});
  if(number===39){result.width=32;result.height=32;result.decorations.push({kind:'paragraph-line',x1:2,x2:30,y:22},{kind:'paragraph-line',x1:2,x2:30,y:30});}
  ...
  const inkHeight=result.height;
  if(inkHeight>32+1e-6)throw Error('Ink height exceeds 32');
  const offset=(44-inkHeight)/2;
  for(const p of result.placements)p.y+=offset;
  for(const d of result.decorations)d.y+=offset;
  result.height=44;
  ...
  records.push({icon_id,number,name:source.name,text,source_id:source.id,source_path:source.reference,author:'gpt-6',width:result.width,height:result.height,ink_height:inkHeight,stroke:4,glyph_ids:result.placements.map(p=>p.glyph.icon_id),line_scales:result.placements.map(p=>p.scale),file:icon_id+'.svg',validation:'Existing glyphs; total ink bounds at most 32 units high; effective stroke 4; natural proportions preserved.'});
...
const icons=records.map(r=>{
 ...
 return {icon_id:r.icon_id,name:r.name,family:'text',profile:'TEXT44',canvas_size:44,canvas_width:r.width,canvas_height:44,max_ink_height:32,text_ink_height:r.ink_height,text:r.text,description:r.name,category:'text',tags:['text','sub icon'],aliases:[],keywords:[r.text],author:r.author,source_ids:[r.source_id],original_sources:[{source_id:r.source_id,source_path:path.relative(root,r.source_path)}],glyph_ids:r.glyph_ids,svg_sha256:crypto.createHash('sha256').update(document).digest('hex'),style:{stroke_width:4,line_cap:'round',line_join:'round'},semantic_role:'SUB',semantic_kind:'text',composition_class:'TEXT',keyshape:null,keyshape_bounds:null,validation:{status:'valid',errors:[],warnings:[],checks_run:['existing glyphs','32-unit maximum ink bounds','4-unit stroke'],scope:'Typeface layout; not SOLO48 geometric validation.'}};
});
const priorPath=path.join(published,'manifest.json');
const prior=fs.existsSync(priorPath)?JSON.parse(fs.readFileSync(priorPath,'utf8')).icons:[];
const freshIds=new Set(icons.map(i=>i.icon_id));
fs.writeFileSync(priorPath,JSON.stringify({schema_version:1,family:'text',profile:'TEXT44',canvas_height:44,icons:[...prior.filter(i=>!freshIds.has(i.icon_id)),...icons]},null,2)+'\n');
