...
with output_lock(dist):
 ...
 for r in accepted:
  ...
  assert new in {r['candidate'] for r in accepted} and qa['svg_sha256']==sha,new
  assert m.STROKE_WIDTH==2 and m.PATH_STROKE_WIDTHS=={'frame':4},new
  ...
  record.update(icon=new,model_key='sub/'+new,python_source=r['candidate_python'],svg=str(target.relative_to(ROOT)),export_url='sub-profiles/'+new+'.svg',sha256=sha,bounds=bounds,model_validation='review',sub32_status='compact_exception',sub32_reason='User-approved 32px composition: 4px frame and 2px inner characters; standard checks retained separately.')
  ...
  record.update(compact_exception_approved=True,stroke_width=2,path_stroke_widths={'frame':4},standard_qa_status=qa['status'])
 ...
 for ent in docs['roles']['icons']:
  ...
  v.update(icon_id=new,python_source=rec['python_source'],svg='icon_set/assets/sub-usage/side/'+new+'.svg',preview_url='sub-usage/side/'+new+'.svg',sha256=rec['sha256'],model_validation='review',compact_exception_approved=True,validated_geometry_sha256=hashlib.sha256(geometry(svg)).hexdigest())
