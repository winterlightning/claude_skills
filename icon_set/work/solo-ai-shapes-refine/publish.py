...
AUTHOR='gpt-6'
w=Path(__file__).parent;rows=json.loads((w/'batch.json').read_text());old=ROOT/'icon_set/work/solo-ai-next100';base=(old/'review-current.html').read_text();current=base
for r in rows:
 for match in re.finditer(r'<article\b[^>]*>.*?</article>',current,re.S):
  ...
  replacement=re.sub(r'<details>.*?</details>',f'<details><summary>Construction notes</summary><p>{html.escape(r["plan"])}</p><p>{r["keyshape"]} · Valid, no warnings</p></details><a href="solo-ai-shapes-refine.html">Compare these revisions ↗</a>',replacement,flags=re.S)
  current=current.replace(card,replacement,1);break
current=current.replace('<div class="toolbar">','<p><a href="solo-ai-shapes-refine.html">Compare the three corrected icons: original, previous and latest ↗</a></p><div class="toolbar">',1)
...
page='<!doctype html><html lang="en"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Solo icons · Subject refinements</title><style>'+css+'</style><body><main><div class="eyebrow">PICTOGRAPHIC / SUBJECT REFINEMENTS</div><h1>Three subjects, restored.</h1><p class="intro">Three corrections: heart-shaped clover leaves, eight cog teeth, and a broader corn cob with overlapping husks. All previous versions are preserved.</p><p><a href="solo-ai-next100.html">Back to all 100 icons ↗</a></p><div class="toolbar"><input id="search" type="search" aria-label="Find an icon" placeholder="Find an icon…"><button id="theme">Dark theme</button><button id="size">Enlarge to 96 px</button><span id="caption">Native size · 48 px</span></div><section class="grid">'+''.join(cards)+'''</section></main><script>
...
(ROOT/'icon_set/dist/gallery/solo-ai-shapes-refine.html').write_text(page)
print('Published three refinements and updated all-100 review.')
