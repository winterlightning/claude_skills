...
class GenerationManager:
    ...
    def build(self, workspace, family, log, dist=None, icon=None):
        ...
        self.command([sys.executable,str(workspace/'icon_set/scripts/build.py'),'--family',family,'--icon',str(workspace/icon),'--no-png','--no-report','--dist',str(dist or build_dist(workspace))],workspace,log)
    ...
    def run(self, row):
        ...
        try:
            ...
            icons=json.loads((build_dist(workspace) / 'gallery/icons.json').read_text())['icons']
            ...
            svg=(build_dist(workspace) / 'gallery'/icon['preview_url']).resolve()
