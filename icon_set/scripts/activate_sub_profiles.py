...
def run(root=ROOT):
    data=root/'icon_set/data';gallery=build_dist(root) / 'gallery'
    ...
    published={}
    for manifest_path in (build_dist(root) / 'sub32/manifest.json', build_dist(root) / 'failed/sub32/manifest.json'):
