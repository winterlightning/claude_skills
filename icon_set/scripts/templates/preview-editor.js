...
window.PreviewIconEditor = function({icons, example}) {
  ...
  let replacements={},counter=0,selected=null,page=0,returnFocus=null,busy=false,variantOn=true;
  ...
  const keyFor=slot=>variantOn?'side:'+slot:slot;
  const baseline=(original,variant)=>(variantOn&&variant)||original;
  function resolve(slot,original,variant){return (variantOn&&replacements['side:'+slot])||(variantOn&&variant)||replacements[slot]||original;}
  ...
  function render(){
    ...
    $('restorePlacement').disabled=!selected||!replacements[keyFor(selected.dataset.slot)]||!byId.has(baseline(selected.dataset.original,selected.dataset.variant));
  }
  ...
  async function open(el){selected=el;returnFocus=el;document.querySelectorAll('.icon-swap.is-selected').forEach(x=>x.classList.remove('is-selected'));el.classList.add('is-selected');$('pickerCurrent').textContent='Replacing '+label(byId.get(el.dataset.icon))+(variantOn?' in the combination view':'')+'. Only this placement will change.';$('iconPickerTitle').textContent=variantOn?'Choose a combined icon':'Choose an approved icon';$('iconPickerSearch').value='';$('iconPickerFamily').value=variantOn?'combination':'';$('iconPickerCategory').value='';page=0;$('pickerResults').replaceChildren();$('restorePlacement').disabled=true;dialog.showModal();$('iconPickerSearch').focus();busy=true;$('pickerResultsStatus').textContent='Loading approved icons…';try{await refreshApproved();if(dialog.open)render();}catch{if(dialog.open)$('pickerResultsStatus').textContent='Could not verify approval. Close the picker and try again.';}finally{busy=false;}}
  ...
  function setVariant(on){variantOn=!!on;document.querySelectorAll('.icon-swap').forEach(el=>apply(el,resolveEl(el)));announce();}
  return {icon,mount,announce,setVariant};
};
