(() => {
  ...
  const operations = [
    ...
    ['Work: claim an icon','POST','/api/work/claim',true,() => ({...current(),worker:'thuan-mac'}),'Disapproved + open/expired → working for 3 hours (lease_hours 1–24). 409 with the current work state when another worker holds it or it is done / cannot-fix.','201 · {saved: true, work: {state: "working", worker, expires_at}, item}'],
    ...
    ['Work: heartbeat','POST','/api/work/heartbeat',true,() => ({...current(),worker:'thuan-mac'}),'Extends your own working lease.','200 · {saved: true, work}'],
    ...
    ['Work: abandon claim','POST','/api/work/abandon',true,() => ({...current(),worker:'thuan-mac'}),'Releases your working claim so another machine can take it. A logged-in reviewer may release anyone’s claim.','200 · {saved: true, work: {state: "open"}}'],
