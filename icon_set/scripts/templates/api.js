(() => {
  ...
  const operations = [
    ...
    ['Disapproved icons with work state','GET','/api/work/disapproved?family=sub&limit=50',false,null,'Every disapproved icon, claimable or not, with its review status (disapprove, claimed) and work state (open, claimed, cannot-fix). Same filters and paging as the queue.','200 · {total, offset, next_offset, items: [{key, svg_sha256, status, reason, feedback, disapproved_by, work}]}'],
    ['Work: current claims','GET',() => '/api/work'+($('apiIcon').value.trim()?iconQuery():''),false,null,'Every claim joined to the current catalog, or one icon’s work state. The claim is the review status: claimed rows with worker and claimed_at; states open, claimed, done, cannot-fix.','200 · {claims: [{icon, svg_sha256, state, worker, note, claimed_at, updated_at, expires_at, current, status}]}'],
    ['Work: claim an icon','POST','/api/work/claim',true,() => ({...current(),worker:'thuan-mac'}),'Disapproved (open) → review status claimed for 6 hours, then Disapproved again on its own; no heartbeat. 409 with the current work state when another worker holds it or it is cannot-fix.','201 · {saved: true, work: {state: "claimed", worker, expires_at}, item}'],
    ...
    ['Work: abandon claim','POST','/api/work/abandon',true,() => ({...current(),worker:'thuan-mac'}),'Sets a claimed or cannot-fix revision back to plain Disapproved so another machine can take it. Anyone may release any claim.','200 · {saved: true, work: {state: "open"}}'],
