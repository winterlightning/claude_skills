(() => {
  ...
  const operations = [
    ...
    ['Disapproved icons with work state','GET','/api/work/disapproved?family=sub&limit=50',false,null,'Every disapproved icon, claimable or not, with its review status (disapprove, claimed) and work state (null, claimed, cannot-fix). Same filters and paging as the queue.','200 · {total, offset, next_offset, items: [{key, svg_sha256, status, reason, feedback, disapproved_by, work}]}'],
    ['Work: current claims','GET',() => '/api/work'+($('apiIcon').value.trim()?iconQuery():''),false,null,'Every claim joined to the current catalog, or one icon’s work state. The claim is the review status: claimed rows with worker and claimed_at; states claimed, done, cannot-fix, or null when nobody worked on it.','200 · {claims: [{icon, svg_sha256, state, worker, note, claimed_at, updated_at, expires_at, current, status}]}'],
    ['Work: claim an icon','POST','/api/work/claim',true,() => ({...current(),worker:'thuan-mac'}),'Disapproved with no work state → review status claimed for 6 hours, then Disapproved again on its own; no heartbeat. 409 with the current work state when another worker holds it or it is cannot-fix.','201 · {saved: true, work: {state: "claimed", worker, expires_at}, item}'],
    ...
    ['Work: cannot fix','POST','/api/work/cannot-fix',true,() => ({...current(),worker:'thuan-mac',note:'Why no meaning-preserving drawing passes.'}),'Gives up with a required note: the icon is Disapproved again but keeps your worker name and note, so the queue skips it until a reviewer decides again or someone abandons it.','200 · {saved: true, work: {state: "cannot-fix"}}'],
