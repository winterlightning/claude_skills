...
def main():
    ...
    rows=json.loads((BASE/'dist/gallery/combinations.json').read_text())['rows']
    ...
    for source in sorted((BASE/'dist/container64').glob('*.svg'),key=lambda p:(p.stem not in mapped,p.stem)):
        ...
        for file,record in grouped.get(ident,{}).items():
            ...
            sub=BASE/'dist/solo48'/f"{record['sub_key'].split('/',1)[1]}.svg"
