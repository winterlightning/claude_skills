...
def run():
    ...
    folder=ROOT/'icon_set/assets/combination-sub32';public=build_dist(ROOT) / 'gallery/combination-sub32'
    ...
    pairs_path.write_text(json.dumps(pairs));(build_dist(ROOT) / 'gallery/experiment-combination.json').write_text(json.dumps(pairs))
    catalog_path=build_dist(ROOT) / 'gallery/combinations.json';catalog=json.loads(catalog_path.read_text())
