...
def plan_source_removal(source_root: Path, icon: dict, indexes: dict | None = None) -> dict:
    ...
    return {'path': path, 'text': text, 'class_name': class_name}


def discard_many(icons: list[dict], *, source_root: Path, dist: Path, archive: Path, connection, user: str) -> dict:
    ...
    for icon in icons:
        try:
            plans.append((icon, plan_source_removal(source_root, icon, indexes)))
