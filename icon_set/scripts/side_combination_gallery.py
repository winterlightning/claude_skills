...
def install_popup(folder, rows, results):
    ...
    import re
    progress=stage(Path(__file__).resolve().parents[1]/'dist/gallery')
    s=re.sub(r'<!-- side-progress:start -->.*?<!-- side-progress:end -->','',s,flags=re.S)
    s=s.replace('<header>','<header>'+compact_html(progress, '../../dist/gallery/sub-repair-review/index.html'),1)
    page.write_text(s)
