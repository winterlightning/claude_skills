...
def main():
    ...
    catalog=json.loads((build_dist(BASE.parent) / 'gallery/combinations.json').read_text());areas=json.loads((BASE/'data/container-content-areas.json').read_text())['areas'];records={r['key']:r for r in json.loads((build_dist(BASE.parent) / 'gallery/icons.json').read_text())['icons']}
    records.update({'container/'+r['icon_id']:r for r in json.loads((build_dist(BASE.parent) / 'container64/manifest.json').read_text())['icons']})
    main_map=json.loads((BASE/'data/container-main-icons.json').read_text())['mappings']
    records.update({'text/'+r['icon_id']:r for r in json.loads((build_dist(BASE.parent) / 'text28/manifest.json').read_text())['icons']})
    ...
    for pair in pairs:
        ...
        if key not in cache:
            host_path=build_dist(BASE.parent) / 'container64'/f'{main_id}.svg';sub_path=build_dist(BASE.parent) / 'text28'/f'{sub["icon_id"]}.svg'
