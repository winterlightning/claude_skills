...
def main(argv=None):
    ...
    names=sorted(p.stem for p in (BASE/'dist/sub32').glob('*.svg')) if args.all_subs else args.sub or DEFAULT_SUBS
    subs={};blocked={}
    for name in names:
        try:
            p=BASE/'dist/sub32'/f'{name}.svg';doc=p.read_text();reject_effects(doc);art=Artwork.read(doc,32)
    ...
    for ident in hosts:
        try:
            path=BASE/'dist/container64'/f'{ident}.svg';digest=sha(path);art=read_art(path.read_text());ink=VectorInk.from_art(art)
            if ident in audit:
                c=audit[ident]
