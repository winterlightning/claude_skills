...
from .path_commands import Command
...
def _validated_commands(raw: object, budget: _Budget) -> list[Command]:
    ...
    for command in raw:
        ...
        if not isinstance(command, Command) or command.type not in counts:
            raise ValueError("Expected normalized path_commands.Command objects (M/L/Q/C/A/Z)")
...
def _contours(paths: object, tolerance: float, budget: _Budget) -> list[_Contour]:
    if not isinstance(paths, list) or not paths:
        raise ValueError("Expected a non-empty list of resolved paths")
    contours = []
    identifiers = set()
    for path in paths:
