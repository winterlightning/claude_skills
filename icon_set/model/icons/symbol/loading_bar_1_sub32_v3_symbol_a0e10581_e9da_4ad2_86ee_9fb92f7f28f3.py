...
class DrawingVariant3ContainerSymbol(Sub32):
    icon_id = 'loading-bar-1-sub32-v3-symbol'
    related_origin_icon_id = 'loading-bar-1-sub32-v3'
    variant_label = 'Independent container symbol'
    usage_category = 'symbol'
    related_group = 'sub-origin/loading-bar-1-sub32-v3'
    counterpart_icon_id = 'loading-bar-1-sub32-v3'
    ...
    def build(self):
        ...
        for i, x in enumerate([8, 18]):
            uid = f'band-{i}'
            self.add_line(uid, (x, 22), (x + 6, 10))
            self.relate('connect', uid, 'top')
            self.relate('connect', uid, 'bottom')
        self.relate('connect', 'band-0', 'left')
        self.relate('connect', 'band-1', 'right')
