...
class ItsukushimaToriiGate(Solo48):
    ...
    def build(self) -> None:
        ...
        roof_y, beam_y, post_x = top + 3, 22, 10
        ...
        self.add_polyline("beam", (left+2,beam_y), (post_x,beam_y), (19,beam_y), (29,beam_y), (48-post_x,beam_y), (right-2,beam_y))
        for side, x in (("left",post_x),("right",48-post_x)):
            self.add_polyline("post-"+side, (x,roof_y), (x,beam_y), (x,bottom))
            self.relate("connect", "roof", "post-"+side)
            self.relate("connect", "beam", "post-"+side)
        for x in (19,29):
            self.add_line(f"inner-{x}", (x,beam_y), (x,bottom))
            self.relate("connect", f"inner-{x}", "beam")
        self.add_polyline("baseline", (6,bottom), (10,bottom), (19,bottom), (29,bottom), (38,bottom), (42,bottom))
        for member in ("post-left","post-right","inner-19","inner-29"):
            self.relate("connect", member, "baseline")
