"""Uppercase I for SOLO48; centerline envelope (10, 4)–(38, 44).
Plan: One centered upright with matching horizontal end bars, distinct from lowercase l.
...
class LetterIUppercase(Solo48):
    icon_id = 'letter-i-uppercase'
    keyshape = Keyshape.VRECT_M
    semantic_role = 'MAIN'
    semantic_kind = 'noun'
    category = 'typeface'
    categories = ('typeface',)
    typeface = {'character': 'I', 'kind': 'uppercase'}
    aliases = ()
    keywords = ('I', 'uppercase', 'capital', 'typeface')

    def build(self):
        rotation = 0
        mirror = False
        axis,left,right=24,10,38
        runs=[((left,y),[('L',axis,y),('L',right,y)],False) for y in (4,44)]
        runs.append(((axis,4),[('L',axis,44)],False))
