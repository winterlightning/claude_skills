...
class ReactLogo(Solo48):
    ...
    def build(self):
        # Plan: Three repeated elliptical orbits rotated by 60 degrees, sharing every true crossing node, around a central dot. Lucide atom informs smooth cubic loops. Parameters own all repeated geometry.
        ...
        for j,a in enumerate(angles):
            ...
            for k,t in enumerate(knots):
                nxt=(k+1)%len(knots);u=knots[nxt]+(2*math.pi if nxt==0 else 0)
                if points[k]==points[nxt]:continue
                ...
                name=f'orbit-{j}-{k}';members.append(name)
                self.add_bezier(name,points[k],(c1,c2,points[nxt]))
            self.add_contour(f'orbit-{j}',*members,closed=True)
        for a,b in [(0,1),(0,2),(1,2)]:self.relate('connect',f'orbit-{a}',f'orbit-{b}')
