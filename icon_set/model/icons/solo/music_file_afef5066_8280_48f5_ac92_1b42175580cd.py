"""A dog-eared file contains a single flagged music note.
Plan: complete reference composition, coherent strokes and parameterized repeat definitions.
SOLO48 VRECT_L; omissions: Fine page corner rounding reduced to round joins.
"""
from ...keyshapes import Keyshape
from ._base import Solo48
SOURCE_ICON_ID='afef5066-8280-48f5-ac92-1b42175580cd'
SOURCE_PATH='icon_set/work/todo-references/music file_afef5066-8280-48f5-ac92-1b42175580cd.svg'
AUTHOR='gpt-6'
class Drawing(Solo48):
    icon_id='music-file'
    keyshape=Keyshape.VRECT_L
    semantic_role='MAIN'
    semantic_kind='noun'
    category='objects'
    aliases=()
    keywords=('music', 'file')

    def circle(self,n,x,y,r):
        self.add_arc(n+'-a',(x-r,y),(x+r,y),radius_x=r)
        self.add_arc(n+'-b',(x+r,y),(x-r,y),radius_x=r)
        self.add_contour(n,n+'-a',n+'-b',closed=True)

    def box(self,n,x,y,w,h,r=2):
        p=[(x+r,y),(x+w-r,y),(x+w,y+r),(x+w,y+h-r),(x+w-r,y+h),(x+r,y+h),(x,y+h-r),(x,y+r)]
        ids=[]
        for i in range(8):
            k=n+'-'+str(i);ids.append(k)
            if i%2:self.add_arc(k,p[i],p[(i+1)%8],radius_x=r)
            else:self.add_line(k,p[i],p[(i+1)%8])
        self.add_contour(n,*ids,closed=True)

    def build(self):

        self.add_polyline('page',(8,4),(30,4),(40,14),(40,44),(8,44),closed=True)
        self.circle('note',21,32,3)
        self.add_line('stem',(24,32),(24,16));self.relate('connect','note','stem')
        self.add_bezier('flag',(24,16),((29,20),(33,22),(29,27)))
        self.relate('connect','stem','flag')

# Final visible bounds: (6, 2, 42, 46)
# Construction: Round noteheads connect to coherent stems and beam runs.
# Final reductions: Fine page corner rounding reduced to round joins.
# Visual review: File corner and flagged note remain identifiable at native size. Contour flow is coherent; flag is intentionally asymmetric.
