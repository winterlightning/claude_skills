...
class MaskedFencerFoil(Solo48):
    icon_id = 'masked-fencer-foil'
    ...
    def build(self):
        ...
        for i in range(4):self.add_arc(f'mask-{i}',pts[i],pts[(i+1)%4],radius_x=10)
        self.add_contour('mask',*[f'mask-{i}' for i in range(4)],closed=True)
        self.skeleton([('vertical',[(16,6),(16,16),(16,26)]),('horizontal',[(6,16),(16,16),(26,16)]),('bust',[(6,42),(6,38),(16,35),(26,32)]),('blade',[(42,6),(32,38),(31,42)])])
        for line,arcs in [('vertical-0',[0,3]),('vertical-1',[1,2]),('horizontal-0',[2,3]),('horizontal-1',[0,1])]:
         for i in arcs:self.relate('connect',line,f'mask-{i}')
        self.add_arc('guard-left',(26,32),(32,38),radius_x=6,sweep=False)
        self.add_arc('guard-right',(32,38),(38,32),radius_x=6,sweep=False)
        self.add_contour('guard','guard-left','guard-right')
        self.relate('connect','bust-2','guard-left')
        for a in ['guard-left','guard-right']:
         for b in ['blade-0','blade-1']:self.relate('connect',a,b)
