"""Uppercase B for SOLO48; centerline envelope (10, 4)–(38, 44).
Plan: Two identical semicircular bowls at pitch 20 attach to one split spine.
...
class LetterBUppercase(Solo48):
    icon_id = 'letter-b-uppercase'
    keyshape = Keyshape.VRECT_M
    semantic_role = 'MAIN'
    semantic_kind = 'noun'
    category = 'typeface'
    typeface = {'character': 'B', 'kind': 'uppercase'}
    aliases = ()
    keywords = ('B', 'uppercase', 'capital', 'typeface')

    def build(self):
        rotation = 0
        mirror = False
        runs=[((10,4),[('L',10,24),('L',10,44)],False)]
        for y in (4,24):
            runs.append(((10,y),[('L',28,y),('A',28,y+20,10,10,True),('L',10,y+20)],False))
