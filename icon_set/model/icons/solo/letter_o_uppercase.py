"""Uppercase O for SOLO48; centerline envelope (10, 4)–(38, 44).
Plan: A single elliptical closed bowl with matching quarters about both axes.
...
class LetterOUppercase(Solo48):
    icon_id = 'letter-o-uppercase'
    keyshape = Keyshape.VRECT_M
    semantic_role = 'MAIN'
    semantic_kind = 'noun'
    category = 'typeface'
    categories = ('typeface',)
    typeface = {'character': 'O', 'kind': 'uppercase'}
    aliases = ()
    keywords = ('O', 'uppercase', 'capital', 'typeface')

    def build(self):
        rotation = 0
        mirror = False
        cx,cy,rx,ry=24,24,14,20
        runs=[((cx-rx,cy),[('A',cx,cy-ry,rx,ry,True),('A',cx+rx,cy,rx,ry,True),('A',cx,cy+ry,rx,ry,True),('A',cx-rx,cy,rx,ry,True)],True)]
