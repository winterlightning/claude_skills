"""The lowercase letter m, re-authored from the supplied reference for SOLO48.

Centerline extremes: left 4, top 8, right 44, bottom 40.
Plan: Two identical arches repeat at a fixed pitch with three aligned stems.
...
SOURCE_ICON_ID = None
SOURCE_PATH = '/Applications/Workspaces/pictographic/claude_skills/Letters/M.svg'
AUTHOR = 'gpt-6'


class LetterM(Solo48):
    icon_id = 'letter-m'
    ...
    aliases = ()
    keywords = ('m', "typeface", "typography", 'lowercase')

    def build(self):
        rotation = 0
        mirror = False
        left, pitch, spring, baseline = 4,20,22,40
        runs = [((left,baseline), [('L',left,spring)],False)]
        for i in range(2):
            x=left+i*pitch
            runs.append(((x,spring), [('A',x+pitch,spring,pitch//2,14,True),('L',x+pitch,baseline)],False))
