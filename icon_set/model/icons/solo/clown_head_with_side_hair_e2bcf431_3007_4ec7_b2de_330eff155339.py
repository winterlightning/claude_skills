"""Clown Head with Side Hair — batch 53."""
from ...keyshapes import Keyshape
from ._base import Solo48
SOURCE_ICON_ID = 'e2bcf431-3007-4ec7-b2de-330eff155339'
SOURCE_PATH = '/Applications/Workspaces/pictographic/icon_simplification/pictographic-primitives/kids/toys clown_e2bcf431-3007-4ec7-b2de-330eff155339.svg'
AUTHOR = 'gpt-6'

class Drawing(Solo48):
    icon_id = 'clown-head-with-side-hair'
    keyshape = Keyshape.SQUARE
    semantic_role = "MAIN"
    semantic_kind = "noun"
    category = 'kids'
    aliases = ()
    keywords = ('clown', 'head', 'with', 'side', 'hair')

    def build(self):
        # Plan: circular blank face, paired three-lobed side hair, two detached wavy collar tails.
        # SQUARE extremes6,6,42,42; user.svg informs circular human face construction.
        # The collar tails are costume marks, not a torso; detach them to preserve hair clearance.
        self.circle('face',24,16,10,attachments=((16,10),(16,22),(32,10),(32,22)))
        for n,s in [('left',1),('right',-1)]:
            def p(x,y):return (24+s*(x-24),y)
            self.add_bezier(n+'-hair',p(16,10),(p(16,6),p(10,6),p(10,12)),(p(6,12),p(6,14),p(6,16)),(p(6,18),p(6,20),p(10,20)),(p(10,26),p(16,26),p(16,22)))
            self.add_bezier(n+'-ribbon',p(18,35),(p(14,36),p(14,37),p(18,38)),(p(20,39),p(18,40),p(16,42)))
        self.join_shared()

    def circle(self,n,x,y,r,attachments=()):
        from math import atan2
        pts=list(dict.fromkeys([(x+r,y),(x,y+r),(x-r,y),(x,y-r)]+list(attachments)))
        pts.sort(key=lambda p:atan2(p[1]-y,p[0]-x))
        for j in range(len(pts)):self.add_arc(f'{n}-{j}',pts[j],pts[(j+1)%len(pts)],radius_x=r)
        self.add_contour(n,*[f'{n}-{j}' for j in range(len(pts))],closed=True)

    def box(self,n,x,y,w,h,r=2):
        pts=[(x+r,y),(x+w//2,y),(x+w-r,y),(x+w,y+r),(x+w,y+h//2),(x+w,y+h-r),(x+w-r,y+h),(x+w//2,y+h),(x+r,y+h),(x,y+h-r),(x,y+h//2),(x,y+r)]
        ids=[]
        for i,(a,z) in enumerate(zip(pts,pts[1:]+pts[:1])):
            if a==z:continue
            name=f'{n}-{i}';ids.append(name)
            if i in (2,5,8,11):self.add_arc(name,a,z,radius_x=r)
            else:self.add_line(name,a,z)
        self.add_contour(n,*ids,closed=True)

    def join_shared(self):
        from icon_set.renderers.svg import build_paths
        paths=build_paths(self.draw())
        for i,a in enumerate(paths):
            ap={(p.start.x,p.start.y) for p in a['primitives']}|{(p.end.x,p.end.y) for p in a['primitives']}
            for z in paths[i+1:]:
                zp={(p.start.x,p.start.y) for p in z['primitives']}|{(p.end.x,p.end.y) for p in z['primitives']}
                if ap & zp:self.relate('connect',a['id'],z['id'])

