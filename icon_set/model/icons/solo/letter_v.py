"""The lowercase letter v, re-authored from the supplied reference for SOLO48.

Centerline extremes: left 6, top 6, right 42, bottom 42.
Plan: Two mirrored diagonal strokes share one lower vertex.
...
SOURCE_ICON_ID = None
SOURCE_PATH = '/Applications/Workspaces/pictographic/claude_skills/Letters/V.svg'
AUTHOR = 'gpt-6'


class LetterV(Solo48):
    icon_id = 'letter-v'
    ...
    typeface = {'character': 'v', 'kind': 'lowercase'}
    aliases = ()
    keywords = ('v', "typeface", "typography", 'lowercase')

    def build(self):
        rotation = 0
        mirror = False
        axis,half_width,top,bottom=24,18,6,42
        runs=[((axis-half_width,top), [('L',axis,bottom),('L',axis+half_width,top)],False)]
