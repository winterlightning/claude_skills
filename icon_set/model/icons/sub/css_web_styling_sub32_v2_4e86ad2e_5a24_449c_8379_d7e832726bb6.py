...
class Drawing(Sub32):
    variant_of = 'css-web-styling-sub32'
    variant_label = 'Uniform 4px readability repair'
    REPAIR_PLAN = {'concept': 'CSS Web Styling Symbol', 'core_parts': ['uppercase C', 'two uppercase S characters in one row'], 'flexible_parts': 'Coordinates and proportions only; no parts removed.', 'repair': 'Increase letter height using the same glyph outlines to open the C and S interiors.'}
    icon_id = 'css-web-styling-sub32-v2'
    ...
    def build(self):
        ...
        self._map_primitives(lambda name: name.startswith('text-'), lambda x, y: (x, 6 + (y - 10) * 20 / 12))
