...
| `build.py` | Validate every icon of every family, then export SVG, PNG preview and a per-family manifest to `dist/<family><canvas>/`. Invalid icons are not shipped: their SVGs and findings go to `dist/failed/<family><canvas>/` and the gallery's Failed build tab (exit code 1); passing icons still publish. Incremental by default: only icons whose source changed since their last output are re-checked; `--all` re-checks everything. Prunes stale outputs. Always runs hole/pinch QA; `debug=False`, `report=True` control evidence. | `python3 icon_set/scripts/build.py` · `--all` · `--family solo` · `--no-png` |
| `failure_report.py` | Render every failed icon from `dist/failed/`, grouped by the rule and violation kind it breaks, with the violation drawn over the icon. The build stages it as `gallery/failures.html` (the Failed build tab). | `python3 icon_set/scripts/failure_report.py` |
