...
class TextFamilyTests(unittest.TestCase):
    def test_text_heights_are_linked_without_changing_existing_exports(self):
        with tempfile.TemporaryDirectory() as folder:
            root = Path(folder)
            target = root/'gallery'
            target.mkdir()
            for height in (28, 32, 44):
            ...
            self.assertEqual([r['preview_url'] for r in rows], ['../text28/text-28.svg', '../text32/text-32.svg', '../text44/text-44.svg'])
