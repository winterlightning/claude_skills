...
(async()=>{
  ...
  assert.equal(fitted.keyshape,'VRECT_M');assert.deepEqual(fitted.scales,{});assert.deepEqual(fitted.offsets,{});
  assert.deepEqual(fitted.geometry[0].start,[8,2]);assert.deepEqual(fitted.geometry.at(-1).end,[24,30]);
  assert.ok(fitted.geometry.every(p=>[...p.start,...p.end].every(Number.isInteger)));
  $('strokeDownload').onclick();const snappedHandoff=JSON.parse(downloadedSVG);assert.deepEqual(snappedHandoff.edited_graph.primitives,fitted.geometry);
  $('strokeDownloadSVG').onclick();assert.match(downloadedSVG,/M8 2/);
  assert.equal($('strokeValidation').dataset.status,'fail');
  ...
  editor.open({...resizeIcon,key:'sub/auto',keyshape:'VRECT_L'});$('editingTab').onclick();await tick();
  $('strokeScope').value='icon';$('strokeScope').onchange();
  assert.equal($('strokeScaleY').value,32);assert.equal($('strokeSave').disabled,true);
  $('strokeCanvas').onkeydown({key:'ArrowRight',preventDefault(){}});$('strokeDownload').onclick();
  assert.deepEqual(JSON.parse(downloadedSVG).edited_graph.primitives[0].start,[9,2]);
  $('strokeUndo').onclick();$('strokeDownload').onclick();
  assert.deepEqual(JSON.parse(downloadedSVG).geometry,fitted.geometry);
  $('strokeResetAll').onclick();assert.equal($('strokeScaleY').value,20);
  ...
  // Centerlines expose editable junctions; shared endpoints and curve handles stay joined.
  const joined={...icon,primitives:[
    {kind:'bezier',element_id:'curve',start:[2,2],end:[12,2],segments:[[[4,2],[8,2],[10,2]],[[10,4],[12,4],[12,2]]]},
    {kind:'line',element_id:'line',start:[12,2],end:[16,2]}],contours:[{contour_id:'joined',members:['curve','line'],closed:false}]};
  const joinedGroup=editor.groups(joined)[0];
  const moved=editor.movePathPoint(joined,joinedGroup,'curve:end',[13,3]);
  assert.deepEqual(JSON.parse(JSON.stringify(moved[0].end)),[13,3]);
  assert.deepEqual(JSON.parse(JSON.stringify(moved[1].start)),[13,3]);
  assert.deepEqual(JSON.parse(JSON.stringify(moved[0].segments[1])),[[10,4],[13,5],[13,3]]);
  const middle=editor.movePathPoint(joined,joinedGroup,'curve:segment:0',[10,3]);
  assert.deepEqual(JSON.parse(JSON.stringify(middle[0].segments)),[[[4,2],[8,3],[10,3]],[[10,5],[12,4],[12,2]]]);
  assert.deepEqual(joined.primitives[0].end,[12,2],'Pure edits preserve the original');
  context.fetch=async()=>response({svg_sha256:'sha',edit:null});
  editor.open({...resizeIcon,key:'sub/points'});$('editingTab').onclick();await tick();
  $('strokePoints').checked=true;$('strokePoints').onchange();$('strokeSnap').checked=true;
  assert.ok(canvas.children.some(e=>e.attrs.class==='editor-centerline'));
  assert.equal(canvas.children.filter(e=>e.attrs['data-point']).length,4,'Closed contour junctions have one handle each');
  // Edit the visible, transformed point; flatten transforms once without jumping.
  canvas.onkeydown({key:'ArrowRight',preventDefault(){}});
  const dot=canvas.children.find(e=>e.attrs['data-point']==='a:start');
  assert.equal(dot.attrs.cx,3);
  canvas.onpointerdown({target:dot,button:0,pointerId:4,clientX:3,clientY:2,preventDefault(){}});
  canvas.onpointermove({pointerId:4,clientX:4.3,clientY:3.1});
  canvas.onpointerup({pointerId:4});
  $('strokeDownload').onclick();let nodeDoc=JSON.parse(downloadedSVG);
  assert.deepEqual(nodeDoc.edited_graph.primitives[0].start,[4,3]);
  assert.deepEqual(nodeDoc.edited_graph.primitives[3].end,[4,3]);
  assert.deepEqual(nodeDoc.edited_graph.primitives[4].start,[16,16]);
  assert.deepEqual(nodeDoc.offsets,{});
  $('strokeUndo').onclick();assert.equal($('strokePointX').value,3);assert.equal($('strokeX').value,1);
  $('strokeRedo').onclick();assert.equal($('strokePointX').value,4);
  canvas.onkeydown({key:'ArrowRight',preventDefault(){}});assert.equal($('strokePointX').value,5);
  $('strokePointX').valueAsNumber=6.3;$('strokePointY').valueAsNumber=3;$('strokePointX').onchange();assert.equal($('strokePointX').value,6);
  const movedDot=canvas.children.find(e=>e.attrs['data-point']==='a:start');
  canvas.onpointerdown({target:movedDot,button:0,pointerId:5,clientX:6,clientY:3,preventDefault(){}});
  canvas.onpointermove({pointerId:5,clientX:7,clientY:4});
  canvas.onpointermove({pointerId:5,clientX:6,clientY:3});
  canvas.onpointerup({pointerId:5});assert.equal($('strokePointX').value,6,'Dragging back to the starting point restores the baseline');
  canvas.onpointerdown({target:movedDot,button:0,pointerId:6,clientX:6,clientY:3,preventDefault(){}});
  canvas.onpointermove({pointerId:6,clientX:8,clientY:5});canvas.onpointercancel({pointerId:6});assert.equal($('strokePointX').value,6);
  let escaped=false;canvas.onkeydown({key:'Escape',preventDefault(){escaped=true;},stopPropagation(){}});assert.equal(escaped,true);assert.equal($('strokePointFields').hidden,true);
  const centerHandle=canvas.children.find(e=>e.attrs['data-center']);
  canvas.onpointerdown({target:centerHandle,button:0,pointerId:7,clientX:10,clientY:5,preventDefault(){}});
  canvas.onpointermove({pointerId:7,clientX:12,clientY:6});canvas.onpointerup({pointerId:7});
  assert.equal($('strokePointFields').hidden,true);assert.equal($('strokeX').value,2);
  $('strokeDownload').onclick();nodeDoc=JSON.parse(downloadedSVG);
  assert.deepEqual(nodeDoc.edited_graph.primitives[0].start,[8,4]);
  let pointSaved;
  context.fetch=async(_url,options)=>{pointSaved={...JSON.parse(options.body),edited_graph:nodeDoc.edited_graph,revision:1};return response(pointSaved);};
  await $('strokeSave').onclick();assert.ok(pointSaved.geometry);
  context.fetch=async()=>response({svg_sha256:'sha',edit:pointSaved});
  editor.open({...resizeIcon,key:'sub/points'});$('editingTab').onclick();await tick();
