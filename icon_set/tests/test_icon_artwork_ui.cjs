...
(async()=>{
 ...
 $('pick_use_upload').onchange();
 context.fetch=async(url,options)=>{body=JSON.parse(options.body);return response({...loaded,choice:{source_mode:'use_upload',revision:2,uploaded:{name:'manual.svg',svg_sha256:'upload'}},record:{...original,artwork_source:'use_upload'}});};
 await $('artworkApply').onclick();assert.equal(body.svg,undefined);assert.equal(body.revision,1);assert.equal(body.source_mode,'use_upload');
