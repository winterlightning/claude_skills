...
def main()->int:
    ...
    for check in doc.get("sourceAnalysis",{}).get("spacingChecks",[]):
        ...
        relation=check.get("relation","ordinary-distinct"); default_width=profile["minimumDistinctCenterlineDistance"] if relation=="ordinary-distinct" else ENVELOPES.get(relation,profile["designStroke"]); width=check.get("envelopeWidth",default_width); offset=len(panels)*panel_width; label=html.escape(" / ".join(check.get("pair",[])))
        ...
        panels.append(f'<g transform="translate({offset} 0)">{context}{selected}<text x="1" y="{canvas+3.4:g}" font-family="sans-serif" font-size="2.4" fill="#111827" stroke="none">{label} · {relation} · {width}u</text></g>')
