---
name: fix-icon-queue
description: Take the next disapproved Pictographic icon from the shared production work queue, fix it, and report the result. Claims the icon on production first so no other machine or agent fixes the same one, then repairs it with the matching family skill, publishes, and reports done (the revision returns to Ready) or cannot-fix. Use when asked to work through disapproved icons, process review feedback, or run the fix queue. Generated from .claude/skills/fix-icon-queue/SKILL.md by icon_set/scripts/generate_skills.py; edit the source, not this copy.
---
