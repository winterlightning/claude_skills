"""Page mail (symbol), converted from the icons-json construction graph by json_to_solo --mode bezier. HRECT_L keyshape; curves kept as cubic beziers."""
from ...keyshapes import Keyshape
from ._base import Solo48

SOURCE_ICON_ID = '2a98051e-e248-4ae8-8a84-a56070dcb2ad'
SOURCE_PATH = 'icons-json/symbol/page mail_2a98051e-e248-4ae8-8a84-a56070dcb2ad.json'
AUTHOR = 'json_to_solo'

class PageMailSymbol(Solo48):
    icon_id = 'page-mail-symbol'
    keyshape = Keyshape.HRECT_L
    semantic_role = 'MAIN'
    semantic_kind = 'noun'
    category = 'symbol'
    aliases = ()
    keywords = ('page', 'mail', 'symbol')

    def build(self):
        self.add_line('e0', (44, 8), (4, 8))
        self.add_line('e1', (44, 8), (24, 30))
        self.add_line('e2', (24, 30), (4, 8))
        self.add_line('e3', (4, 8), (4, 40))
        self.add_line('e4', (4, 40), (44, 40))
        self.add_line('e5', (44, 8), (44, 40))
        self.add_contour('c0', 'e0')
        self.add_contour('c1', 'e1', 'e2', 'e3', 'e4')
        self.add_contour('c2', 'e5')
        self.relate('connect', 'c0', 'c1')
        self.relate('connect', 'c0', 'c2')
        self.relate('connect', 'c1', 'c2')
        self.relate('connect', 'c0', 'c1')
