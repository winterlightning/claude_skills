...
| --- | --- |
| SVGs and family manifests | `published/sub32/`, `solo48/`, `container64/` |
| PNG previews | `published/previews-png/<family><canvas>/` |
| Web app assets and icon catalog | `published/gallery/` |
| Failed SVGs and findings | `published/failed/<family><canvas>/` |
| QA HTML and measurements | `published/qa/` (ignored by Git, regenerated) |
...
The default HTML report is `published/qa/index.html`, available from the
...
`icon_set.validation.library_qa.load_results(Path("published/qa"))`.
...
| `icon_set/metadata/` | Curated source metadata, tracked in Git |
| `icon_set/data/` | Supporting datasets (combination pairs, container trials), tracked in Git |
| `icon_set/work/`, `icon_set/assets/` | Experiments, reference SVGs and experiment snapshots, tracked in Git |
| `published/` | The one build root: exports, gallery, failed builds, reports, previews; tracked and served on production |
| `published/qa/` | Per-icon validation evidence, regenerated by every build, ignored by Git |
| `icon_set/state/` | Local review database, backups, uploads, edits and agent jobs, ignored by Git |
...
python3 -m icon_set publish        # then commit published/ with the sources

# Optional: an immutable release outside the checkout.
...
Every command uses `published/` directly; there is no separate development
build or staging copy. Default builds use Python originals, do not seed source
metadata, and do not refresh datasets in place. The production server overlays
its saved manual choices at request time. Export does not copy the development
database. For an update or rollback, change the release path and retain the
production state path.

The retired `icon_set/.local/`, `icon_set/dist/` and `icon_set/assets/previews-png/`
folders are kept on disk only and nothing reads them. Historical work reports may
still link there; active scripts and generated authoring skills use `published/`.
