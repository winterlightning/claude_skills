...
window.PreviewIconEditor = function({icons, example}) {
  ...
  function resolveEl(el){return replacements[el.dataset.slot]||(variantOn&&el.dataset.variant)||el.dataset.original;}
  function icon(id,cls='',slot,variant){
    slot=slot||'position-'+counter++;
    const current=byId.get(replacements[slot])||byId.get((variantOn&&variant)||id);
  ...
  async function open(el){selected=el;returnFocus=el;document.querySelectorAll('.icon-swap.is-selected').forEach(x=>x.classList.remove('is-selected'));el.classList.add('is-selected');$('pickerCurrent').textContent='Replacing '+label(byId.get(el.dataset.icon))+'. Only this placement will change.';$('iconPickerSearch').value='';$('iconPickerFamily').value='';$('iconPickerCategory').value='';page=0;$('pickerResults').replaceChildren();$('restorePlacement').disabled=true;dialog.showModal();$('iconPickerSearch').focus();busy=true;$('pickerResultsStatus').textContent='Loading approved icons…';try{await refreshApproved();if(dialog.open)render();}catch{if(dialog.open)$('pickerResultsStatus').textContent='Could not verify approval. Close the picker and try again.';}finally{busy=false;}}
