(() => {
  ...
  const operations = [
    ...
    ['Work queue: claimable disapproved icons','GET','/api/work/queue?family=sub&limit=20',false,null,'Disapproved icons no machine is fixing (no work state), oldest first. Filters: family, category, type, limit 1–500, offset. Development servers forward this to production.','200 · {total, offset, next_offset, items: [{key, svg_sha256, python_source, reason, feedback, disapproved_by, work}]}'],
    ['Disapproved icons with work state','GET','/api/work/disapproved?family=sub&limit=50',false,null,'Every disapproved icon, claimable or not, with its review status (disapprove, claimed) and work state (null, working or cannot-fix); add state=cannot-fix to list the icons workers gave up on. Same filters and paging as the queue.','200 · {total, offset, next_offset, items: [{key, svg_sha256, status, reason, feedback, disapproved_by, work}]}'],
    ['Work: current claims','GET',() => '/api/work'+($('apiIcon').value.trim()?iconQuery():''),false,null,'Every claim joined to the current catalog, or one icon’s work state. The claim is the review status: claimed rows with worker and claimed_at; states working (status claimed), done (status ready), cannot-fix (still disapproved, worker and note kept), or null when nobody worked on it.','200 · {claims: [{icon, svg_sha256, state, worker, note, claimed_at, updated_at, expires_at, current, status}]}'],
    ['Work: uploaded fixes shown in the gallery','GET','/api/work/fixes',false,null,'Every uploaded after result by revision. While an icon’s current revision has one, the gallery shows that SVG (artwork_source work_fix) instead of the Python drawing; svg_sha256 stays the Python revision, and a rebuilt model replaces it. ?variant=use_org on /api/icon-artwork/svg still returns the original.','200 · {fixes: [{icon, svg_sha256, worker, saved_at}]}'],
    ['Work: claim an icon','POST','/api/work/claim',true,() => ({...current(),worker:'thuan-mac'}),'Disapproved with no work state → review status claimed for 6 hours, then Disapproved again on its own; no heartbeat. 409 with the current work state when another worker holds it or it is cannot-fix.','201 · {saved: true, work: {state: "working", worker, expires_at}, item}'],
    ['Work: claim several icons','POST','/api/work/claim',true,() => ({worker:'thuan-mac',icons:[{icon:current().icon,svg_sha256:current().svg_sha256},'sub/another']}),'Claims each listed icon on its own: entries are icon keys or {icon, svg_sha256} objects (a key alone uses production’s current hash). Up to 500 per call.','200 · {saved, worker, claimed: [item…], refused: [{icon, status, error, work}]}'],
    ['Work: report done → Ready','POST','/api/work/done',true,() => ({...current(),worker:'thuan-mac',note:'sub/plus-v3'}),'Marks your claim done and returns this revision to Ready for the reviewer. Disapproval feedback is kept. Re-disapproving later reopens the icon for claiming.','200 · {saved: true, status: "ready", work: {state: "done"}}'],
    ['Work: cannot fix','POST','/api/work/cannot-fix',true,() => ({...current(),worker:'thuan-mac',note:'Why no meaning-preserving drawing passes.'}),'Gives up with a required note: the icon stays Disapproved with your worker name and note, the queue skips it, and reviewers filter it with state=cannot-fix or the Cannot fix reason in the gallery.','200 · {saved: true, work: {state: "cannot-fix"}}'],
    ['Work: abandon claim','POST','/api/work/abandon',true,() => ({...current(),worker:'thuan-mac'}),'Sets a claimed or cannot-fix revision back to plain Disapproved so another machine can take it. Anyone may release any claim.','200 · {saved: true, work: {state: null}}'],
